...
if (isset($_POST['edit_id']) && is_numeric($_POST['edit_id'])) {
    ...
    $res = CAddressing::GetStateAllLangs($edit_id);
} else {
    $action = 'add';
    $edit_id = null;
    $res = [];
...
<div class="row">
    <div class="col-md-10">
        ...
        <form action="index.php?menu=post&submenu=action" method="POST" id="add_state_form">
                ...
                <?php foreach (CLanguage::get_langsUser() as $key => $lang) {
                    ...
                    <div role="tabpanel" class="tab-pane <?= $active_class ?>" id="<?= $lang['key'] ?>_tabnav">
                        <div class="form-inline address-name-input">
                            <input type="text" name="lang[<?= $lang['key'] ?>]" class="form-control" placeholder="<?= CDictionary::GetKey('state_name') ?>"  value="<?php if($res) echo $res['text'][$lang['key']] ?>">
                        </div>
                    </div>
            ...
            <button type="button" class="btn btn-success" id="add_state"><?= CDictionary::GetKey($action) ?></button>
        </form>
    </div>
</div>
            ...
            <?php foreach ($states as $key => $state) { ?>
                <tr>
                    ...
                    <?php foreach (CLanguage::get_lang_keys_user() as $item) { ?>
                        <td><?= $state['text'][$item]  ?></td>
                    <?php } ?>


                    <td><?= $state['addr_time'] ?></td>
                    <td><?= $state['addr_price'] ?></td>
                    <td><?= $state['addr_tax'] ?></td>
                    <td><?= $state['addr_tel_code'] ?></td>
                    <td><?= $state['addr_zip'] ?></td>
                    <td><?= $state['addr_order'] ?></td>
                    <td>
                        <button class="btn btn-default" data-action="edit-state" data-value="<?= $key ?>"><i
...
<script>
    $(function () {
        $('#add_state').on('click', function () {
            ...
            var f_data = $('#add_state_form').serialize();
            $.ajax({
