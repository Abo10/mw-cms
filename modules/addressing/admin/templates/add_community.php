...
if (isset($_POST['edit_id']) && is_numeric($_POST['edit_id'])) {
    ...
    $res = CAddressing::GetCommunityAllLangs($edit_id);
} else {
    $action = 'add';
    $edit_id = null;
    $res = [];
...
CModule::LinkModule('addressing');
$communities = CAddressing::GetCommunitiesAllLangs($city);
?>
...
<div class="row">
    <div class="col-md-10">
        ...
        <form action="index.php?menu=post&submenu=action" method="POST" id="add_community_form">
                ...
                <?php foreach (CLanguage::get_langsUser() as $key => $lang) {
                    ...
                    <div role="tabpanel" class="tab-pane <?= $active_class ?>" id="<?= $lang['key'] ?>_tabnav">
                        <div class="form-inline address-name-input">
                            <input type="text" class="form-control" placeholder="<?= CDictionary::GetKey('community_name') ?>"  name="lang[<?= $lang['key'] ?>]" value="<?php if($res) echo $res['text'][$lang['key']] ?>">
                        </div>
                    </div>
            ...
            <button class="btn btn-success" type="button" id="add_community"><?= CDictionary::GetKey($action) ?></button>
        </form>
    </div>
</div>
            ...
            <?php foreach ($communities as $key => $community) { ?>
                <tr>
                    ...
                    <?php foreach (CLanguage::get_lang_keys_user() as $item) { ?>
                        <td><?= $community['text'][$item] ?></td>
                    <?php } ?>
                    <td><?= $community['addr_time'] ?></td>
                    <td><?= $community['addr_price'] ?></td>
                    <td><?= $community['addr_tax'] ?></td>
                    <td><?= $community['addr_tel_code'] ?></td>
                    <td><?= $community['addr_zip'] ?></td>
                    <td><?= $community['addr_order'] ?></td>
                    <td>
                        <button class="btn btn-default" data-action="edit-community" data-value="<?= $key ?>"><i
...
<script>
    $(function () {
        $('#add_community').on('click', function () {
            ...
            var f_data = $('#add_community_form').serialize();
            $.ajax({
